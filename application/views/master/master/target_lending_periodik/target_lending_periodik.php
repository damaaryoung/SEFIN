...
<!-- data target lending periodik -->
<div id="lihat_target_lending_periodik" class="content-wrapper" style="padding-left: 15px; padding-right: 15px;">
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Target Lending Periodik</h1>
                </div>
                <div class="col-sm-6">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="box-body table-responsive no-padding">
                            <button class="btn btn-primary tambah" id="click_tambah_target_lending_periodik" style="margin-bottom: 9px;"><i class="fa fa-user-plus">Tambah</i></button>
                            <table id="example2" class="table table-bordered table-hover table-sm" style="white-space: nowrap;">
                                ...
                                <tbody id="data_target_lending_periodik" style="font-size: 13px">
                                </tbody>
                            </table>
                        </div>
                    </div>
...
<!-- tambah target lending periodik -->
<div id="tambah_target_lending_periodik" class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Tambah Target Lending Periodik</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">Home</a></li>
                        <li class="breadcrumb-item">Target Lending Periodik</li>
                        <li class="breadcrumb-item active">Tambah Target Lending Periodik</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-md-12">
                    <div class="card card-primary">
                        <div class="card-header">
                                <!-- <h3 class="card-title">Tambah Target Lending Periodik</h3> -->
                            </div>
                        <form role="form" id="form_tambah_target_lending_periodik">
                            <div class="card-body">
...
var target_persen = document.getElementById('target_persen');
    ...
    function submit() {
        ...
        if (requestBody.periode == "") {
            ...
            $.ajax({ 
                type : 'POST',
                url : 'Target_lending_periodik_controller/submit',
                data :  requestBody,
                cache: false,
                success : function(res) {
                    ...
                    for (var key in jsonResponse.detail_hk) {
                        ...
                        arrayAjax.push(tambah_target_lending_periodik(formData));
                    }
                    ...
                    Promise.all(arrayAjax).then((result) => {
                        ...
                        if (flagSuccess) {
                            ...
                            $('#form_tambah_target_lending_periodik')[0].reset();
                            hide_all();
                            $('#lihat_target_lending_periodik').show();
                            load_data();
                        } else {
    ...
    function confirmDelete(bulan, tahun, periode) {
        bootbox.confirm({
            ...
            callback: function(result){
                if(result){
                    delete_target_lending_periodik(bulan, tahun, periode)
                    .done(function(res){
                        // console.log(res);
                        load_data();
                    })
    ...
    $(function(){
        ...
        hide_all = function(){
            $('#lihat_target_lending_periodik').hide();
            $('#tambah_target_lending_periodik').hide();
        }
        ...
        $('#click_tambah_target_lending_periodik').click(function(){
            hide_all();
            $('#tambah_target_lending_periodik').show();
        });

        tambah_target_lending_periodik = function(opts){
            var url = '<?php echo $this->config->item('api_url');?>api/master/MasterActivity/periodik';
            var data = opts;
        ...
        delete_target_lending_periodik = function(bulan, tahun, periode){
            var url = '<?php echo $this->config->item('api_url');?>api/master/MasterActivity/periodik/delete/'+bulan+'/'+tahun+'/'+periode;
        ...
        get_target_lending_periodik = function(opts, id){
            var url = '<?php echo config_item('api_url') ?>api/master/MasterActivity/indexperiodik';
        ...
        load_data = function(pagin){
            ...
            .done(function(response){
                ...
                if(data.length === 0 ){
                    ...
                    $('#data_target_lending_periodik').html(tr);
                ...
                $('#data_target_lending_periodik').html(html);
                $('#example2').DataTable({
                    "paging": true,
                    "retrieve": true,
                    "lengthChange": true,
            ...
            .fail(function(response){
                $('#data_target_lending_periodik').html('<tr><td colspan="8" style="text-align: center">Tidak ada data</td></tr>');
            });
        }

        $('#data_target_lending_periodik').show();
        load_data();
    });
