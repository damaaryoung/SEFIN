                 	...
                 	<?php foreach($output_fid_ever_cabang as $res):?>
                    	<div class="col-md-6">
				            <div class="card">
                                            ...
                                            <div class="btn-group">
                                              ...
                                              <div class="dropdown-menu dropdown-menu-right" role="menu">
                                                <div class="dropdown-item">
                                                  <input type="date" class="form-control" name="date_fid_ever_console_cabang_<?php echo str_replace(" ","",$res->nama_area_kerja);?>">
                                                    </div>
                                                  <div class="dropdown-item">
                                                    <button class="btn btn-success btn-block btn-sm" href="javascript:void(0)" id="preview_list_fid_ever_console_cabang_<?php echo str_replace(" ","",$res->nama_area_kerja);?>">Lihat Data</button>
                                                  </div>
                                                </div>
                                              </div>
				              	...
				              	<div class="card-body">
                    				<div id="FID_EVER_CONSOLE_CABANG_<?php echo str_replace(" ","",$res->nama_area_kerja);?>" style="width:400px;height:400px"></div>
                    			</div>
                    		</div>
                    	</div>
                    <?php endforeach ?>
...
$('input[name="date_fid_ever_console_cabang_<?php echo str_replace(" ","",$row->nama_area_kerja); ?>"]').change(function(){
  var date = $('input[name="date_fid_ever_console_cabang_<?php echo str_replace(" ","",$row->nama_area_kerja) ?>"]').val();
  var data = {
    'api' : 'Y',
    'tgl' : date,
  ...
  $.ajax({
    ...
    success: function(result){
      var chart = new CanvasJS.Chart("FID_EVER_CONSOLE_CABANG_<?php echo str_replace(" ","",$row->nama_area_kerja);?>", {
          title: {
...
$('#preview_list_fid_ever_console_cabang_<?php echo str_replace(" ","",$row->nama_area_kerja) ?>').click(function(){
  var date = $('input[name="date_fid_ever_console_cabang_<?php echo str_replace(" ","",$row->nama_area_kerja) ?>"]').val();
