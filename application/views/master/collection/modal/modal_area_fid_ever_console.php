             ...
             <div class="modal-body">
                <div class="row">
                    <input type="hidden" name="kode_area" id="kode_area" value=""/>
                 	<?php foreach($output_fid_ever_area as $res):?>
                    	<div class="col-md-6">
				            <div class="card">
                                ...
                                <div class="btn-group">
                                  ...
                                  <div class="dropdown-menu dropdown-menu-right" role="menu">
                                    <div class="dropdown-item">
                                      <input type="date" class="form-control" name="date_fid_ever_console_area_<?php echo $res->kode_area;?>">
                                        </div>
                                      <div class="dropdown-item">
                                        <button class="btn btn-success btn-block btn-sm" href="javascript:void(0)" id="preview_list_fid_ever_console_area_<?php echo $res->kode_area;?>">Lihat Data</button>
                                      </div>
                                    </div>
                                  </div>
				              	...
				              	<div class="card-body">
                    				<div id="FID_EVER_CONSOLE_AREA_<?php echo $res->kode_area;?>" style="width:400px;height:400px"></div>
                                    <button type="button" class="btn btn-md btn-danger col-12" id="btn_fid_ever_<?php echo $res->kode_area;?>" data-toggle="modal" onclick="get_cabang_per_area('<?php echo $res->kode_area;?>')">Tampilkan Cabang</button>
                                    <div id="div_fid_ever_cabang_<?php echo $res->kode_area;?>"></div>
                    			</div>
                    		</div>
                    	</div>
                    <?php endforeach ?>
...
<?php $i=0; foreach($output_fid_ever_area as $row):?>
    ...
    url = urlapi + "/dashboard/kredit/kreditrisk_controller/fid_ever_06_console_area";

    var chart = new CanvasJS.Chart("FID_EVER_CONSOLE_AREA_" + kode_area, {
        title: {
            text: kode_area,
            fontSize: 12
        },
...
$('input[name="date_fid_ever_console_area_<?php echo $row->kode_area ?>"]').change(function(){
  var date = $('input[name="date_fid_ever_console_area_<?php echo $row->kode_area ?>"]').val();
  var data = {
    'api' : 'Y',
    'tgl' : date,
  ...
  $.ajax({
    ...
    success: function(result){
      var chart = new CanvasJS.Chart("FID_EVER_CONSOLE_AREA_<?php echo $row->kode_area;?>", {
          title: {
              text: kode_area,
              fontSize: 12
          },
...
$('#preview_list_fid_ever_console_area_<?php echo $row->kode_area ?>').click(function(){
  var date = $('input[name="date_fid_ever_console_area_<?php echo $row->kode_area ?>"]').val();
  var url = "<?php echo base_url();?>modal_bootstrap_controller/modal_list_fid_ever_console_area";
  var data = {
    'api' : 'Y',
    'tgl' : date,
    'kode_area' : "<?php echo $row->kode_area ?>"
  ...
  $.ajax({
    ...
    success: function(response){
      ...
      $('#date_fid_ever_console_area').val(date);
...
function get_cabang_per_area(area){
  var date = $('input[name="date_fid_ever_console"]').val();
    ...
    //alert(data);
    $.ajax({  
      url: "<?php echo base_url();?>modal_bootstrap_controller/modal_cabang_fid_ever_console",
      data: data,
      type: "POST",
      beforeSend: function(){
        $('#modal-2').modal('show');
      ...
      success: function(response)
      {
        ...
        $('.title-chart-cabang').text('Daftar FID EVER Cabang ' + area);
