...
class Ol_controller extends CI_Controller
{
    ...
    function get_data_caa()
    {
        ...
        foreach ($list as $field) {
            ...
            $url = "index.php/report/Memo_pfka";

            $row = array();

            $row[] = '<form method="post" target="_blank" action="' . $url . '"> <button type="button" title="Edit OL" class="btn btn-info btn-sm edit"   data-target="#update" data="' . $id . '"><i class="fas fa-pen"></i></button>
            <input type="hidden" name ="id" value="' . $id . '"><button type="submit" title="Report OL" class="btn btn-success btn-sm" ><i class="far fa-file-pdf"></i></a></form>';
            $row[] = $nomor_so;
            $row[] = $field->cabang;
