...
class Export extends CI_Controller
{
  ...
  public function export()
  {
    ...
    if ($keperluan == 'IDEB') {
      ...
      if ($area == 'SEMUA AREA' and $cabang == 'SEMUA CABANG') {
        $query = "SELECT b.nomor_so, b.created_at AS tgl_pengajuan, a.created_at AS tgl_pembuatan_caa, c.nama_lengkap AS nama_debitur, d.plafon, d.tenor, b.nama_so, b.nama_marketing, e.nama AS asal_data, d.jenis_pinjaman, g.recom_produk_kredit AS produk, g.recom_nilai_pinjaman AS approval_pinjaman,h.nama AS area, i.nama AS cabang, j.nama AS nama_ca, a.status_team_caa FROM trans_caa AS a LEFT JOIN trans_so AS b ON b.id=a.id_trans_so LEFT JOIN calon_debitur AS c ON b.id_calon_debitur=c.id LEFT JOIN fasilitas_pinjaman AS d ON d.id=b.id_fasilitas_pinjaman LEFT JOIN master_asal_data AS e ON e.id=b.id_asal_data LEFT JOIN trans_ca AS f ON f.id_trans_so=b.id LEFT JOIN rekomendasi_pinjaman AS g ON f.id_rekomendasi_pinjaman=g.id LEFT JOIN mk_area AS h ON h.id=a.id_area LEFT JOIN mk_cabang AS i ON i.id=a.id_cabang LEFT JOIN m_pic AS j ON j.id= a.id_pic WHERE a.status_caa = '1' AND (b.created_at BETWEEN '$dari_tgl' AND '$sampai_tgl' + interval 1 day)";
      } else if ($area != 'SEMUA AREA' and $cabang == 'SEMUA CABANG' and $dari_tgl1 != '' and $sampai_tgl != '') {
        $query = "SELECT b.nomor_so, b.created_at AS tgl_pengajuan, a.created_at AS tgl_pembuatan_caa, c.nama_lengkap AS nama_debitur, d.plafon, d.tenor, b.nama_so, b.nama_marketing, e.nama AS asal_data, d.jenis_pinjaman, g.recom_produk_kredit AS produk, g.recom_nilai_pinjaman AS approval_pinjaman,h.nama AS area, i.nama AS cabang, j.nama AS nama_ca, a.status_team_caa FROM trans_caa AS a LEFT JOIN trans_so AS b ON b.id=a.id_trans_so LEFT JOIN calon_debitur AS c ON b.id_calon_debitur=c.id LEFT JOIN fasilitas_pinjaman AS d ON d.id=b.id_fasilitas_pinjaman LEFT JOIN master_asal_data AS e ON e.id=b.id_asal_data LEFT JOIN trans_ca AS f ON f.id_trans_so=b.id LEFT JOIN rekomendasi_pinjaman AS g ON f.id_rekomendasi_pinjaman=g.id LEFT JOIN mk_area AS h ON h.id=a.id_area LEFT JOIN mk_cabang AS i ON i.id=a.id_cabang LEFT JOIN m_pic AS j ON j.id= a.id_pic WHERE a.status_caa = '1' AND h.nama ='$area'  AND (b.created_at BETWEEN '$dari_tgl' AND '$sampai_tgl' + interval 1 day)";
      } else if ($area != 'SEMUA AREA' and $cabang != 'SEMUA CABANG' and $dari_tgl1 != '' and $sampai_tgl != '') {
        $query = "SELECT b.nomor_so, b.created_at AS tgl_pengajuan, a.created_at AS tgl_pembuatan_caa, c.nama_lengkap AS nama_debitur, d.plafon, d.tenor, b.nama_so, b.nama_marketing, e.nama AS asal_data, d.jenis_pinjaman, g.recom_produk_kredit AS produk, g.recom_nilai_pinjaman AS approval_pinjaman,h.nama AS area, i.nama AS cabang, j.nama AS nama_ca, a.status_team_caa FROM trans_caa AS a LEFT JOIN trans_so AS b ON b.id=a.id_trans_so LEFT JOIN calon_debitur AS c ON b.id_calon_debitur=c.id LEFT JOIN fasilitas_pinjaman AS d ON d.id=b.id_fasilitas_pinjaman LEFT JOIN master_asal_data AS e ON e.id=b.id_asal_data LEFT JOIN trans_ca AS f ON f.id_trans_so=b.id LEFT JOIN rekomendasi_pinjaman AS g ON f.id_rekomendasi_pinjaman=g.id LEFT JOIN mk_area AS h ON h.id=a.id_area LEFT JOIN mk_cabang AS i ON i.id=a.id_cabang LEFT JOIN m_pic AS j ON j.id= a.id_pic WHERE a.status_caa = '1' AND h.nama ='$area' AND i.nama='$cabang' AND (b.created_at BETWEEN '$dari_tgl' AND '$sampai_tgl' + interval 1 day)";
      }
      $data = $this->db->query($query)->result();
      ...
      foreach ($data as $hasil) {
        ...
        ->setCellValue('O' . $kolom, $hasil->cabang)
        ->setCellValue('P' . $kolom, $hasil->status_team_caa);
