...
class Export extends CI_Controller
{
  ...
  public function export()
  {
    ...
    if ($keperluan == 'IDEB') {
      ...
      if ($area == 'SEMUA AREA' and $cabang == 'SEMUA CABANG') {
        $query = "  SELECT b.nomor_so, b.created_at AS tgl_pengajuan, a.created_at AS tgl_pembuatan_ca, c.nama_lengkap AS nama_debitur, d.plafon, d.tenor, r.nama AS nama_ca, b.nama_marketing, e.nama AS asal_data, d.jenis_pinjaman, i.recom_produk_kredit AS produk, i.recom_nilai_pinjaman AS plafon_recomend, CONCAT(c.alamat_domisili,' RT.', c.rt_domisili,' RW.', c.rw_domisili,' Provinsi:',j.nama,' Kabupaten:', k.nama,' Kecamatan:',l.nama,' Kelurahan:',m.nama,' Kode Pos:',m.kode_pos) AS alamat_domisili, CONCAT(t.alamat,' RT.', t.rt,' RW.', t.rw,' Provinsi:',n.nama,' Kabupaten:', o.nama,' Kecamatan:',p.nama,' Kelurahan:',q.nama,' Kode Pos:',q.kode_pos) AS alamat_jaminan, a.status_ca, i.note_recom AS catatan_ca, f.nama AS area, g.nama AS cabang FROM trans_ca AS a LEFT JOIN trans_so AS b ON b.id=a.id_trans_so LEFT JOIN calon_debitur AS c ON b.id_calon_debitur=c.id  LEFT JOIN fasilitas_pinjaman AS d ON b.id_fasilitas_pinjaman=d.id LEFT JOIN master_asal_data AS e ON b.id_asal_data=e.id LEFT JOIN mk_area AS f ON b.id_area=f.id LEFT JOIN mk_cabang AS g ON b.id_cabang=g.id LEFT JOIN trans_ao AS h ON b.id=h.id_trans_so LEFT JOIN rekomendasi_pinjaman AS i ON a.id_rekomendasi_pinjaman=i.id LEFT JOIN agunan_tanah AS t ON h.id_agunan_tanah=t.id LEFT JOIN m_pic AS r ON a.id_pic=r.id LEFT JOIN master_provinsi AS j ON j.id = c.id_prov_domisili LEFT JOIN master_kabupaten AS k ON k.id = c.id_kab_domisili LEFT JOIN master_kecamatan AS l ON l.id = c.id_kec_ktp LEFT JOIN master_kelurahan AS m ON m.id = c.id_kel_domisili LEFT JOIN master_provinsi AS n ON n.id = t.id_provinsi LEFT JOIN master_kabupaten AS o ON o.id = t.id_kabupaten LEFT JOIN master_kecamatan AS p ON p.id = t.id_kecamatan LEFT JOIN master_kelurahan AS q ON q.id = t.id_kelurahan WHERE (b.created_at BETWEEN '$dari_tgl' AND '$sampai_tgl' + interval 1 day)";
      } else if ($area != 'SEMUA AREA' and $cabang == 'SEMUA CABANG' and $dari_tgl1 != '' and $sampai_tgl != '') {
        $query = "SELECT b.nomor_so, b.created_at AS tgl_pengajuan, a.created_at AS tgl_pembuatan_ca, c.nama_lengkap AS nama_debitur, d.plafon, d.tenor, r.nama AS nama_ca, b.nama_marketing, e.nama AS asal_data, d.jenis_pinjaman, i.recom_produk_kredit AS produk, i.recom_nilai_pinjaman AS plafon_recomend, CONCAT(c.alamat_domisili,' RT.', c.rt_domisili,' RW.', c.rw_domisili,' Provinsi:',j.nama,' Kabupaten:', k.nama,' Kecamatan:',l.nama,' Kelurahan:',m.nama,' Kode Pos:',m.kode_pos) AS alamat_domisili, CONCAT(t.alamat,' RT.', t.rt,' RW.', t.rw,' Provinsi:',n.nama,' Kabupaten:', o.nama,' Kecamatan:',p.nama,' Kelurahan:',q.nama,' Kode Pos:',q.kode_pos) AS alamat_jaminan, a.status_ca, i.note_recom AS catatan_ca, f.nama AS area, g.nama AS cabang FROM trans_ca AS a LEFT JOIN trans_so AS b ON b.id=a.id_trans_so LEFT JOIN calon_debitur AS c ON b.id_calon_debitur=c.id  LEFT JOIN fasilitas_pinjaman AS d ON b.id_fasilitas_pinjaman=d.id LEFT JOIN master_asal_data AS e ON b.id_asal_data=e.id LEFT JOIN mk_area AS f ON b.id_area=f.id LEFT JOIN mk_cabang AS g ON b.id_cabang=g.id LEFT JOIN trans_ao AS h ON b.id=h.id_trans_so LEFT JOIN rekomendasi_pinjaman AS i ON a.id_rekomendasi_pinjaman=i.id LEFT JOIN agunan_tanah AS t ON h.id_agunan_tanah=t.id LEFT JOIN m_pic AS r ON a.id_pic=r.id LEFT JOIN master_provinsi AS j ON j.id = c.id_prov_domisili LEFT JOIN master_kabupaten AS k ON k.id = c.id_kab_domisili LEFT JOIN master_kecamatan AS l ON l.id = c.id_kec_ktp LEFT JOIN master_kelurahan AS m ON m.id = c.id_kel_domisili LEFT JOIN master_provinsi AS n ON n.id = t.id_provinsi LEFT JOIN master_kabupaten AS o ON o.id = t.id_kabupaten LEFT JOIN master_kecamatan AS p ON p.id = t.id_kecamatan LEFT JOIN master_kelurahan AS q ON q.id = t.id_kelurahan WHERE f.nama='$area' AND (b.created_at BETWEEN '$dari_tgl' AND '$sampai_tgl' + interval 1 day)";
      } else if ($area != 'SEMUA AREA' and $cabang != 'SEMUA CABANG' and $dari_tgl1 != '' and $sampai_tgl != '') {
        $query = "SELECT b.nomor_so, b.created_at AS tgl_pengajuan, a.created_at AS tgl_pembuatan_ca, c.nama_lengkap AS nama_debitur, d.plafon, d.tenor, r.nama AS nama_ca, b.nama_marketing, e.nama AS asal_data, d.jenis_pinjaman, i.recom_produk_kredit AS produk, i.recom_nilai_pinjaman AS plafon_recomend, CONCAT(c.alamat_domisili,' RT.', c.rt_domisili,' RW.', c.rw_domisili,' Provinsi:',j.nama,' Kabupaten:', k.nama,' Kecamatan:',l.nama,' Kelurahan:',m.nama,' Kode Pos:',m.kode_pos) AS alamat_domisili, CONCAT(t.alamat,' RT.', t.rt,' RW.', t.rw,' Provinsi:',n.nama,' Kabupaten:', o.nama,' Kecamatan:',p.nama,' Kelurahan:',q.nama,' Kode Pos:',q.kode_pos) AS alamat_jaminan, a.status_ca, i.note_recom AS catatan_ca, f.nama AS area, g.nama AS cabang FROM trans_ca AS a LEFT JOIN trans_so AS b ON b.id=a.id_trans_so LEFT JOIN calon_debitur AS c ON b.id_calon_debitur=c.id  LEFT JOIN fasilitas_pinjaman AS d ON b.id_fasilitas_pinjaman=d.id LEFT JOIN master_asal_data AS e ON b.id_asal_data=e.id LEFT JOIN mk_area AS f ON b.id_area=f.id LEFT JOIN mk_cabang AS g ON b.id_cabang=g.id LEFT JOIN trans_ao AS h ON b.id=h.id_trans_so LEFT JOIN rekomendasi_pinjaman AS i ON a.id_rekomendasi_pinjaman=i.id LEFT JOIN agunan_tanah AS t ON h.id_agunan_tanah=t.id LEFT JOIN m_pic AS r ON a.id_pic=r.id LEFT JOIN master_provinsi AS j ON j.id = c.id_prov_domisili LEFT JOIN master_kabupaten AS k ON k.id = c.id_kab_domisili LEFT JOIN master_kecamatan AS l ON l.id = c.id_kec_ktp LEFT JOIN master_kelurahan AS m ON m.id = c.id_kel_domisili LEFT JOIN master_provinsi AS n ON n.id = t.id_provinsi LEFT JOIN master_kabupaten AS o ON o.id = t.id_kabupaten LEFT JOIN master_kecamatan AS p ON p.id = t.id_kecamatan LEFT JOIN master_kelurahan AS q ON q.id = t.id_kelurahan WHERE f.nama='$area' and g.nama='$cabang' AND (b.created_at BETWEEN '$dari_tgl' AND '$sampai_tgl' + interval 1 day)";
      }
      $data = $this->db->query($query)->result();
      ...
      $spreadsheet->setActiveSheetIndex(0)
        ...
        ->setCellValue('N1', 'Approval Pinjaman');
