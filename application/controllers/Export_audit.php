...
class Export_audit extends CI_Controller
{
  ...
  public function export()
  {
    ...
    if ($keperluan == 'CA') {
      ...
      if ($area == 'SEMUA AREA' and $cabang == 'SEMUA CABANG' and $dari_tgl1 == '' and $sampai_tgl == '') {
        $query = "SELECT a.table, a.ip, a.agents, b.nama as nama_pic, d.nama as nama_area, c.nama as nama_cabang, a.message, a.updated_time, a.jenis from sefin_activity_log a LEFT JOIN dpm_online.`user` b
        ON (a.user_id = b.user_id) join mk_cabang c on(a.id_cabang=c.id) join mk_area d on (a.id_area=d.id)";
      } else if ($area == 'SEMUA AREA' and $cabang == 'SEMUA CABANG' and $dari_tgl1 != '' and $sampai_tgl != '') {
        $query = "SELECT a.table, a.ip, a.agents, b.nama as nama_pic, d.nama as nama_area, c.nama as nama_cabang, a.message, a.updated_time, a.jenis from sefin_activity_log a LEFT JOIN dpm_online.`user` b
        ON (a.user_id = b.user_id) join mk_cabang c on(a.id_cabang=c.id) join mk_area d on (a.id_area=d.id) WHERE DATE(a.updated_time) BETWEEN '$dari_tgl' AND '$sampai_tgl' + interval 1 day";
      } else if ($area != 'SEMUA AREA' and $dari_tgl1 != '' and $sampai_tgl != '') {
        $query = "SELECT a.table, a.ip, a.agents, b.nama as nama_pic, d.nama as nama_area, c.nama as nama_cabang, a.message, a.updated_time, a.jenis from sefin_activity_log a LEFT JOIN dpm_online.`user` b
        ON (a.user_id = b.user_id) join mk_cabang c on(a.id_cabang=c.id) join mk_area d on (a.id_area=d.id) WHERE d.nama='$area' AND DATE(a.updated_time) BETWEEN '$dari_tgl' AND '$sampai_tgl' + interval 1 day";
      } else if ($area != 'SEMUA AREA' and $cabang != 'SEMUA CABANG' and $dari_tgl1 != '' and $sampai_tgl != '') {
        $query = "SELECT a.table, a.ip, a.agents, b.nama as nama_pic, d.nama as nama_area, c.nama as nama_cabang, a.message, a.updated_time, a.jenis from sefin_activity_log a LEFT JOIN dpm_online.`user` b
        ON (a.user_id = b.user_id) join mk_cabang c on(a.id_cabang=c.id) join mk_area d on (a.id_area=d.id) WHERE d.nama='$area' AND c.nama='$cabang' AND DATE(a.updated_time) BETWEEN '$dari_tgl' AND '$sampai_tgl' + interval 1 day";
      }
      ...
      $spreadsheet->setActiveSheetIndex(0)
        ->setCellValue('A1', 'No')
        ->setCellValue('B1', 'Table')
        ->setCellValue('C1', 'IP Address')
        ->setCellValue('D1', 'Access Data From')
        ->setCellValue('E1', 'Nama PIC')
        ->setCellValue('F1', 'Area')
        ->setCellValue('G1', 'Kantor')
        ->setCellValue('H1', 'Perubahan Data')
        ->setCellValue('I1', 'Waktu Update')
        ->setCellValue('J1', 'Jeni Data');
      ...
      foreach ($data as $hasil) {
        ...
        $sheet = $spreadsheet->getActiveSheet()
          ...
          ->setCellValue('B' . $kolom, $hasil->table)
          ->setCellValue('C' . $kolom, $hasil->ip)
          ->setCellValue('D' . $kolom, $hasil->agents)
          ->setCellValue('E' . $kolom, $hasil->nama_pic)
          ->setCellValue('F' . $kolom, $hasil->nama_area)
          ->setCellValue('G' . $kolom, $hasil->nama_cabang)
          ->setCellValue('H' . $kolom, $hasil->message)
          ->setCellValue('I' . $kolom, $hasil->updated_time)
          ->setCellValue('J' . $kolom, $hasil->jenis);
      ...
      $filename = 'Export Data Log Activity CA SEFIN';
