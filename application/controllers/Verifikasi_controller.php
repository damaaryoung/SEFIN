...
class Verifikasi_controller extends CI_Controller
{
    ...
    function export_verifikasi($bulan='', $tahun='', $jenis_verifikasi) {

        if($jenis_verifikasi == "hasil_verifikasi") {
            ...
            $query_error = "SELECT 
                ...
                a.npwp AS npwp,
                a.nop AS nop,
                c.nama AS nama_verif,
                a.messages AS pesan,
                a.rc AS status_call,
                a.created_at AS waktu
            FROM vj_api_neg_case AS a
            LEFT JOIN mk_cabang AS b ON b.id = a.id_cabang
            LEFT JOIN dpm_online.user AS c ON a.user_id = c.user_id
            ...
            $this->load->view('master/verifikasi/verifikasi_telepon');
        }
